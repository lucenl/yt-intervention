...
if __name__ == "__main__":
    ...
    if args["steps"] == "intervention":
        if not os.path.exists(puppet_state_file):
            logging.error(f"Puppet state file {puppet_state_file} does not exist")
            sys.exit(1)
        if not os.path.exists(profile_dir) or not os.listdir(profile_dir):
            logging.error(f"Profile directory {profile_dir} is missing or empty")
            sys.exit(1)
        ...
        logging.info(f"No pre-trained puppets...Initializing new puppet")
        puppet = init_puppet(args["puppetId"], profile_dir)
