...
def train():
    ...
    for videoId in training_videos:
        ...
        try:
            ...
            if watched == trainingN - 1:
                ...
                while (not up_next or len(up_next) < 12) and retry_upnext < MAX_RETRY:
                    # Try once more if we got empty recommendations
                    puppet['driver'].driver.refresh()
                    time.sleep(2)
                    retry_upnext += 1
                    # Take screenshot before getting recommendations
                    puppet['driver'].save_screenshot(os.path.join(
                        screenshots_dir, f"last_video_retry_{retry_upnext}.png"))
                    up_next = puppet['driver'].get_upnext_recommendations(topn=12)
    ...
    while (not homepage or len(homepage) < 25) and retry_homepage < MAX_RETRY:
        # Try once more if we got empty recommendations
        time.sleep(2)
        homepage = puppet['driver'].get_homepage_recommendations(scroll_times=10)
        retry_homepage += 1
        # Take screenshot after first attempt
        puppet['driver'].save_screenshot(os.path.join(screenshots_dir, f"homepage_retry_{retry_homepage}.png"))
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Successfully created profile directory")
        init_puppet(args['puppetId'], profile_dir)
        print("Initialized sock puppet:", args['puppetId'])
        ...
        #         trainWatch()
        #     if 'searchAndWatch' in args['steps']:
        #         search_and_watch()
        # finalize puppet
